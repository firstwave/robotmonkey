...
public class SeeMonkeyRobot implements IRobot {
	   protected IChimpDevice _device;
	   protected Location _mouse;
	   protected int _autoDelay = 0;

	   public SeeMonkeyRobot(IChimpDevice device){
	      _device = device;
	   ...
	   public void key(boolean down, int keycode){
	   ...
	   public ScreenImage captureScreen(Rectangle screenRect) {
		   ...
		   try{
			   ...
			   bimg = bimg.getSubimage(screenRect.x, screenRect.y, screenRect.width, screenRect.height);
